 ...
 *               target_id:
 *                 type: integer
 *                 description: ID de la cible (anime ou manga)
 *               target_type:
 *                 type: string
 *                 enum: [anime, manga]
...
app.post('/api/animes/:id/relations', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const relationColumn = target_type === 'anime' ? 'id_anime' : 'id_manga';
    const relationValue = target_id;
    const ficheDepart = `anime${id}`;
    const checkQuery = `
      SELECT id_relation FROM ak_fiche_to_fiche 
      WHERE id_fiche_depart = $1 AND ${relationColumn} = $2
    `;
    const existing = await pool.query(checkQuery, [ficheDepart, relationValue]);
    ...
    if (target_type === 'anime') {
      insertQuery = `
        ...
        RETURNING *
      `;
      insertParams = [ficheDepart, target_id];
    } else {
      insertQuery = `
        INSERT INTO ak_fiche_to_fiche (id_fiche_depart, id_anime, id_manga, id_ost, id_jeu, id_business)
        VALUES ($1, 0, $2, 0, 0, 0)
 ...
 *   delete:
 *     summary: Supprimer une relation d'un anime
 ...
 *     responses:
 *       200:
 *         description: Liste des screenshots récupérée avec succès
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   type: string
 *                   format: binary
 *                 description: Fichiers image des screenshots
 *               id_titre:
 *                 type: integer
 *                 description: ID de l'anime
 *               type:
 *                 type: integer
 *                 description: Type des images (1 pour les screenshots, 2 pour les couvertures)
 *     responses:
 *       201:
 *         description: Screenshots uploadés avec succès
 *       400:
 *         description: Données invalides
 *       401:
 *         description: Authentification requise
 ...
 *         schema:
 *           type: integer
 *         description: ID du screenshot
 *     responses:
 *       200:
 *         description: Screenshot supprimé avec succès
 *       404:
 *         description: Screenshot non trouvé
 *       401:
 *         description: Authentification requise
 *       403:
 *         description: Accès admin requis
...
app.delete('/api/admin/animes/:id/screenshots/:screenshotId', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const fs = require('fs');
    const path = require('path');
    const filePath = path.join(__dirname, '..', 'frontend', 'public', 'images', screenshotInfo.rows[0].filename);
