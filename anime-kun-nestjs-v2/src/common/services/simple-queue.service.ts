...
export class SimpleQueueService {
  ...
  async addReviewModerationJob(reviewId: number, userId: number, contentId: number, contentType: string) {
  ...
  async addContentReportJob(reportId: number, contentType: string, contentId: number, reason: string, priority: 'low' | 'normal' | 'high' = 'normal') {
  ...
  async addBulkModerationJob(
    ...
    console.log(`Adding bulk moderation job for ${targetIds.length} ${targetType}s`);
  ...
  // Clean up old jobs
  async cleanOldJobs(maxAge: number = 24 * 60 * 60 * 1000) { // 24 hours
    const cutoff = new Date(Date.now() - maxAge);
    
    for (const [id, job] of this.jobs.entries()) {
      if (job.created_at < cutoff && (job.status === 'completed' || job.status === 'failed')) {
        this.jobs.delete(id);
      }
    }
  }
