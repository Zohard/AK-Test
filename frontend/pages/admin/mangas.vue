      ...
      <table class="admin-table">
        ...
        <tbody>
          <tr v-for="manga in mangas" :key="manga.id_manga" class="table-row">
            <td class="image-cell">
              <img
                :src="manga.image ? `/images/${manga.image}` : '/placeholder-manga.jpg'"
                :alt="manga.titre"
            ...
            <td class="actions-cell">
              <button @click="editManga(manga)" class="action-btn edit-btn">
                ✏️
              </button>
    ...
    <div v-if="showCreateModal || showEditModal" class="modal-overlay" @click="closeModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
          <h2>{{ isEditing ? 'Modifier le manga' : 'Ajouter un manga' }}</h2>
...
const editManga = (manga) => {
  ...
  formData.value = {
    ...
    titre: manga.titre || '',
    auteur: manga.auteur || '',
    annee: manga.annee || new Date().getFullYear(),
    nb_volumes: manga.nb_volumes || null,
    synopsis: manga.synopsis || '',
    image: manga.image || '',
    statut: Boolean(manga.statut)
  }
...
const saveManga = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      await $fetch(`${API_BASE}/api/admin/mangas/${formData.value.id}`, {
        method: 'PUT',
        headers: authStore.getAuthHeaders(),
        body: payload
      })
